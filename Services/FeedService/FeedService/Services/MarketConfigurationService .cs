...
namespace FeedService.Services
{
    public class MarketConfigurationService(
        ILogger<MarketConfigurationService> _logger,
        INorceClient _norceClient,
        IOptionsMonitor<NorceBaseModuleOptions> _norceOptions) : IMarketConfigurationService
    {

        public async Task<List<MarketConfiguration>> GetMarketConfigurations(Guid? traceId = null)
        {
            try
            {
        ...
        /// </exception>
        private Task<MarketConfiguration?> MapCultureToMarketConfiguration(ClientCulture culture, Guid? traceId = null)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "TraceId: {traceId} Service: {serviceName} LogType: {logType} Method: {method} Error Source: {errorSource} Error Message: {errorMessage} Error Stacktrace: {errorStackTrace} Error Inner Exception: {errorInnerException} Internal Message: {internalMessage}| Other Parameters {cultureCode}", traceId, nameof(MarketConfigurationService), nameof(LoggingTypes.ErrorLog), nameof(MapCultureToMarketConfiguration), ex.Source, ex.Message, ex.StackTrace, ex.InnerException, "An unexpected error occurred while mapping ClientCulture to MarketConfiguration", culture.CultureCode);
